...
function c4Index(row, col) {
    return row * 7 + col;
}
function detectC4Winner(cells) {
    for (let r = 0; r < 6; r++) {
        for (let c = 0; c < 4; c++) {
            const v = cells[c4Index(r, c)];
            if (v !== 0 && v === cells[c4Index(r, c + 1)] && v === cells[c4Index(r, c + 2)] && v === cells[c4Index(r, c + 3)])
                return v;
        }
    }
    for (let c = 0; c < 7; c++) {
        for (let r = 0; r < 3; r++) {
            const v = cells[c4Index(r, c)];
            if (v !== 0 && v === cells[c4Index(r + 1, c)] && v === cells[c4Index(r + 2, c)] && v === cells[c4Index(r + 3, c)])
                return v;
        }
    }
    for (let r = 0; r < 3; r++) {
        for (let c = 0; c < 4; c++) {
            const v = cells[c4Index(r, c)];
            if (v !== 0 && v === cells[c4Index(r + 1, c + 1)] && v === cells[c4Index(r + 2, c + 2)] && v === cells[c4Index(r + 3, c + 3)])
                return v;
        }
    }
    for (let r = 3; r < 6; r++) {
